...
const RomLibrary = (() => {
  ...
  const IGNORE_EXTENSIONS = new Set([
    "txt", "nfo", "md", "pdf", "doc", "docx",
    "jpg", "jpeg", "png", "gif", "bmp",
    "zip", "rar", "7z",
    "dat", "cue", "m3u", "sav", "state", "srm",
    "xml", "json", "db", "ini", "url", "cfg",
    "a", "b", "c", // stray split-archive fragments, not real ROMs
  ]);

  function looksLikeSplitFragment(ext) {
    // Catches old split-archive-style suffixes like .0a, .1b, .001,
    // .r00, etc. - these are leftover file fragments, not real ROMs.
    return /^\d+[a-z]?$/.test(ext) || /^r\d+$/.test(ext);
  }

  function guessSystem(path, filename) {
    const ext = filename.split(".").pop().toLowerCase();
    if (IGNORE_EXTENSIONS.has(ext) || looksLikeSplitFragment(ext)) return null;
    ...
    // VISIBLE_SYSTEMS in app.js), so a genuinely ambiguous extension
    // (like a plain .bin with no folder hint) defaults to Atari
    // rather than being silently skipped. Known non-ROM file types
    // are already filtered out above, so this only catches real
    // uncertain ROM-like files.
    return "atari2600";
  }
  ...
  function baseKey(filename) {
    // Same idea as normalizeId, but also strips ALL (region/version)
    // tags AND every separator character entirely (not just
    // collapsing them), so "3-D Tic-Tac-Toe", "3 D Tic Tac Toe", and
    // "3D Tic Tac Toe" all reduce to the same key for deduplication,
    // regardless of spacing/punctuation differences between copies.
    return filename
      .replace(/\.[^.]+$/, "")
      .replace(/\s*\([^)]*\)\s*/g, "")
      .toLowerCase()
  ...
  function versionScore(filename) {
    ...
    let score = 0;
    if (lower.includes("(usa)")) score += 100;
    else if (lower.includes("(world)")) score += 80;
    else if (lower.includes("(europe)")) score += 60;
    if (/\((proto|beta|demo|hack|unl)/.test(lower)) score -= 50;
    score -= filename.length * 0.01; // prefer shorter/cleaner filenames as a tiebreaker
    return score;
  }
  ...
  // ---------- recursive scan ----------
  async function scanDir(dirHandle, dedupeMap, path = "") {
    for await (const [name, handle] of dirHandle.entries()) {
      const fullPath = path ? `${path}/${name}` : name;
      if (handle.kind === "directory") {
        await scanDir(handle, dedupeMap, fullPath);
      } else {
        ...
        const key = system + "::" + baseKey(name);
        const score = versionScore(name);
        const existing = dedupeMap.get(key);
        if (existing && existing.score >= score) continue; // keep the better version already found
  ...
  async function loadFrom(dirHandle) {
    ...
    await scanDir(dirHandle, dedupeMap);
    libraryGames = Array.from(dedupeMap.values());
    connected = true;
  }

  // Try to silently reconnect using the remembered handle. Returns
  // "connected", "needs-permission", or "none". If the folder was
  // moved/deleted since we last saw it, the handle itself throws
  // instead of just denying permission - catch that and treat it the
  // same as "never connected," so the app falls back to showing the
  // Choose ROM Folder button instead of crashing on load.
  async function tryReconnect() {
    ...
    try {
      ...
      console.warn("Saved ROM folder is no longer valid (moved/deleted?):", e);
      return "none";
    }
  }
